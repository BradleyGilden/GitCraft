...
const downBtn3 = document.getElementById('download-btn3');
const select2 = document.getElementById('dev-type2');
const viewLink2 = document.getElementById('view-param2');
...
select2.addEventListener('change', function() {
  ...
  // Update the href attribute of the link with the selected value
  viewLink2.href = viewLink2.dataset.select + selectedValue;
});
...
downBtn3.addEventListener('click', async function() {
  const queryString = new URLSearchParams({param1: select.value}).toString();
  const loadingAnimation = `
  <span class="spinner-border spinner-border-sm" aria-hidden="true"></span>
  <span role="status">Downloading...</span>
  `;
  downBtn3.innerHTML = loadingAnimation;
  const response = await fetch(`${downBtn3.dataset.download}?${queryString}`, {
    method: 'GET',
    headers: {'Content-Type': 'application/zip'}
  });
  const blob = await response.blob();
